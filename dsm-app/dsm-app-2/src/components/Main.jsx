...
export default function Main({ user }) {
  ...
  const handleSubmitCheckin = async () => {
    ...
    const { data: cd } = await supabase.from('weekly_checkins').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).limit(8)
    setCheckinHistory(cd || [])
    alert('✅ Weekly check-in submitted to Coach V!')
  }
    ...
    <div style={C.app}>
      ...
      {tab === 'home' && (
        <div style={C.scroll} className="fade">
          <div style={C.orange}>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', fontWeight: 700 }}>— Coach V</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
            {[['🤖','COACH V','AI coach','bot'],['🎥','COURSE','Video lessons','course']].map(([icon,label,sub,t]) => (
      ...
      {tab === 'actions' && (
        <div style={C.scroll} className="fade">
          ...
          <div style={{ ...C.orange }}>
            ...
            <div style={{ fontSize:14,fontWeight:800,lineHeight:1.4 }}>Fill this out after EVERY practice and game. It goes straight to Coach V. 🦈</div>
          </div>
          <div style={C.card}>
          ...
          {checkinDone ? (
            <div>
              <div style={{ ...C.card,borderColor:'#1a4a1a',textAlign:'center',padding:36,marginBottom:16 }}>
                ...
                <div style={{ fontSize:13,color:'#555' }}>Coach V has your check-in. See you next week. 🔥</div>
              </div>
            ...
            <div style={C.card}>
              ...
              <span style={C.lbl}>💬 MESSAGE TO COACH V</span>
              <textarea style={{ ...C.ta,height:65 }} placeholder="Anything you want Coach V to know?" value={checkin.messageToCoach} onChange={e=>setCI('messageToCoach',e.target.value)} />
            </div>
      ...
      {tab === 'bot' && (
        <div style={{ display:'flex',flexDirection:'column',height:'calc(100vh - 116px)' }} className="fade">
          <div style={{ padding:'12px 20px 10px',borderBottom:'1px solid #1a1a1a',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
            <div><div style={C.title}>COACH V</div><div style={C.sub}>YOUR AI MINDSET COACH</div></div>
          ...
          <div style={{ flex:1,overflowY:'auto',padding:'12px 20px' }}>
            {messages.length === 1 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 9, color: '#555', letterSpacing: 3, fontWeight: 700, marginBottom: 10 }}>ASK COACH V</div>
      ...
      {tab === 'dashboard' && isCoach && !selectedAthlete && (
        <div style={C.scroll} className="fade">
          ...
          <div style={C.sub}>COACH VIEW</div>
